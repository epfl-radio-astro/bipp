...
# Telescope Name
parser.add_argument("-t", "--telescope", type=str, required=True, choices=['SKAlow', 'LOFAR', 'MWA'],
                    help="Name of telescope.\n Currently SKALow, LOFAR and MWA accepted.")
# Path to MS File
parser.add_argument("-ms", "--ms_file", type=str, required=True,
...
parser.add_argument("-c", "--channel", nargs=2, type=int,
...
skip_count = 0
with bipp.DatasetFile.create(args.output, args.telescope.lower(), N_antenna, N_station) as dataset:
    for t, f, S in ProgressBar(
            ms.visibilities(channel_id=channel_id, time_id=slice(timeStart, timeEnd, timeStep), column=args.data)
    ):
