...
struct StatusMessage {
  ...
  static auto recv(const MPICommHandle& comm)
      -> std::variant<RootCommDestroyed, CreateSynthesis, SynthesisStep>;
};
...
auto StatusMessage::send_synthesis_collect(const MPICommHandle& comm, std::size_t id,
    ...
    -> void {
  StatusMessage m;
  m.messageIndex = StatusMessage::SynthesisStep::index;
...
auto StatusMessage::recv(const MPICommHandle& comm)
  ...
  switch (m.messageIndex) {
    ...
    case StatusMessage::CreateSynthesis::index:
      return m.create;
    case StatusMessage::SynthesisStep::index:
      return m.step;
    default:
...
auto recv_synthesis_collect_data(const MPICommHandle& comm,
  ...
  if (ctx.processing_unit() == BIPP_PU_GPU) {
    ...
    v = queue.create_pinned_array<std::complex<T>, 2>(ctx.host_alloc(), {info.nAntenna, info.nEig});
    dMasked = queue.create_pinned_array<T, 2>(ctx.host_alloc(), {info.nEig, syn.image().shape(1)});
        ...
        [&](auto&& info) {
          ...
          if constexpr (std::is_same_v<T, StatusMessage::RootCommDestroyed>) {
            ...
            if (info.typeSize == sizeof(float)) {
              ...
              synthesisFloat.emplace(info.id,
                                     recv_synthesis_init_data<float>(comm_, ctx, info));
            } else {
              if (synthesisDouble.count(info.id))
                throw InternalError("MPI Create Synthesis: Duplicate synthesis id");
              synthesisDouble.emplace(info.id,
                                      recv_synthesis_init_data<double>(comm_, ctx, info));
            }
          } else if constexpr (std::is_same_v<T, StatusMessage::SynthesisStep>) {
